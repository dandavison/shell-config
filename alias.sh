...
alias -g _b='$(f-git-select-branch)'
alias -g _bc='$(f-git-select-commit)'
#alias -g cf='$(f-git-select-file)'
alias bb='gc _b'
alias batz='bat $(fzf)'
alias cargo-test-fzf='f-cargo-test'
alias gcpz='git cherry-pick "$(f-git-select-branch)"'
alias gcz='git checkout --quiet "$(f-git-select-branch)"'
alias gdmz='git diff origin/main "$(fzf)"'
alias gdz='git diff "$(fzf)"'
alias glz='_gl "$(f-git-select-branch)"'
alias grhz='git reset --hard "$(f-git-select-commit)"'
alias griz='git rebase --interactive "$(f-git-select-commit)"'
alias grsz='git reset "$(f-git-select-commit)"'
alias grvz='git revert --no-edit "$(f-git-select-commit)"'
alias grz='git rebase "$(f-git-select-branch)"'
alias gstaz='git stash apply $(f-git-stash-list)'
alias gstz='f-git-stash'
alias gsz='git show "$(f-git-select-commit)"'
alias hz='f-hist-cp'
alias hzx='f-hist-x'
...
alias claude-usage-breakdown='npx ccusage daily --instances --json --since "$(date +%Y%m01)" | jq -r '"'"'.projects | to_entries | map({project: (.key), cost: ([.value[].totalCost] | add | . * 100 | round / 100)}) | sort_by(-.cost) | ["PROJECT","COST"], (.[] | [.project, ("$" + (.cost | tostring))]) | @tsv'"'"' | column -t'
alias clipboard-restart='killall pboard'
alias comp='load-completions'
alias count='sort | uniq -c | sort -rn'
alias cw='cat-which'
alias d='delta-toggle'
alias delta-walkthrough='LESS="-FIRX -pWALKTHROUGH" delta'
alias di='docker images'
alias dl='delta-toggle line-numbers'
alias dm='delta-maintenance'
...
alias gbo='git-branch-overwrite'
...
alias gdsm='gd --stat=200,200 origin/main...'
...
alias git-make-main='git fetch origin main && git branch -d main && git branch main origin/main'
...
alias le='less'
alias lg='lazygit'
alias list-commands='print -l -- ${(ko)commands}'
alias ll='fdl'
alias lp='glp'
...
alias pytest-quiet='uv run pytest --no-header --tb=short --disable-warnings --log-level=ERROR --log-cli-level=ERROR'
alias pytest='uv run pytest --no-header'
